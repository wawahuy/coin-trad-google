...
export default async function superSync(req: Request, res: Response) {
  ...
  const model = await ModelWorker.findOne({ _id: new Types.ObjectId(id), type: WorkerType.SuperManager });
  if (model) {
    let dataNew: IWorker = {};
    ...
    await ModelWorker.updateOne({ _id: new Types.ObjectId(id), type: WorkerType.SuperManager }, data);
    res.json({
      status: true,
      data: model
    });
