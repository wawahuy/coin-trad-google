...
import fs from 'fs';
import * as superService from '../services/super';
...
async function takeNewSession() {
  ...
  try {
    if (context.id) {
      const d = await superService.sync(context.id, {});
  ...
  if (workerCurrent < workerMax) {
    log('session get...');
    const workerNext = await coinService.getOne().catch(e => null);
    ...
    if (workerNextData?.status) {
      ...
      if (session) {
        ...
        if (result == SessionStatus.Next) {
          ...
          coinService.checkpoint(workerID);
        } else {
